...
import SellerSignUpModal from './signup/signup';
...
const TravelWebsite = () => {
    ...
    return (
        <div className="min-h-screen bg-gray-900 text-white font-sans antialiased">
            {/* Show ProfileSection as a modal/overlay if userProfile is set */}
            {/* {userProfile && ( */}
                <profileSection
                    // user={userProfile}
                        ...
                        <nav className="hidden md:flex items-center space-x-8">
                            <button
                                ...
                                className={`text-gray-300 hover:text-white transition-colors px-2 py-1 rounded 
                                    ${activePage === 'home' ? 'border-b-2 border-blue-500 text-white' : ''}`}
                            >
                                Home
                            </button>
                            <button
                                ...
                                className={`text-gray-300 hover:text-white transition-colors px-2 py-1 rounded 
                                    ${activePage === 'explore' ? 'border-b-2 border-blue-500 text-white' : ''}`}
                            >
                                Explore
                            </button>
                            <button
                                ...
                                className={`text-gray-300 hover:text-white transition-colors px-2 py-1 rounded 
                                    ${activePage === 'medical' ? 'border-b-2 border-blue-500 text-white' : ''}`}
                            >
                                Medical Services
                            </button>
                            <button
                ...
                {isMenuOpen && (
                    <div className="md:hidden bg-gray-900 absolute w-full left-0 py-4 shadow-lg animate-fade-in-down">
                        <div className="flex flex-col items-center space-y-4">
                            ...
                            <button onClick={() => { setIsSignUpModalOpen(true); setIsMenuOpen(false); }}
                                className="w-4/5 bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition-colors">Sign Up</button>
            ...
            <main className="py-12">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <section className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
                        ...
                        <button onClick={() => setIsPriceListModalOpen(true)}

                            className="bg-gray-800 rounded-xl shadow-lg p-6 text-center transform transition-all duration-300 hover:scale-105 hover:shadow-2xl">
                        ...
                        <button
                            className="bg-gray-800 rounded-xl shadow-lg p-6 text-center transform transition-all duration-300 hover:scale-105 hover:shadow-2xl"
                        >
                            <HelpCircleIcon className="h-12 w-12 text-blue-400 mx-auto mb-4" />
                            <h3 className="text-xl font-semibold text-white">Need Help?</h3>
                            <p className="text-gray-400 mt-2">Get quick access to tourist helpline.</p>
                        </button>
                        <button
                            className="bg-gray-800 rounded-xl shadow-lg p-6 text-center transform transition-all duration-300 hover:scale-105 hover:shadow-2xl"
                        >
...
{/* 
                    {isPriceListModalOpen && (
                        ...
                        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4 animate-fade-in">
                            ...
                            <div className="bg-gray-800 rounded-xl max-w-lg w-full p-6 shadow-2xl transform transition-transform duration-300 scale-100">

                                <div className="flex justify-between items-center mb-4">

                                    <h3 className="text-xl font-semibold text-white">Local Price Guide</h3>

                                    <button onClick={() => setIsPriceListModalOpen(false)} className="text-gray-400 hover:text-white">&times;</button>

                                </div>

                                <ul className="space-y-4">
                                    {priceList.map((item, index) => (
                                        <li key={index} className="flex justify-between items-center pb-2 border-b border-gray-700">
                                            <div>
                                                <p className="font-semibold text-gray-200">{item.service}</p>
                                                <p className="text-sm text-gray-400">{item.details}</p>
                                            </div>
                                            <span className="font-bold text-lg text-green-400">{item.price}</span>
                                        </li>
                                    ))}
                                </ul>
                                <p className="text-xs text-gray-500 mt-4">Note: Prices are approximate and may vary.</p>
                            </div>
                        </div>
                    ...
                    {/* {isSOSModalOpen && (
                        <div className="fixed inset-0 bg-red-600 bg-opacity-90 flex items-center justify-center z-50 p-4 animate-fade-in">
                            <div className="bg-white rounded-xl max-w-lg w-full p-8 text-center shadow-2xl transform transition-transform duration-300 scale-100">
                                <div className="flex justify-end mb-4">
                                    <button onClick={() => setIsSOSModalOpen(false)} className="text-gray-500 hover:text-gray-700">&times;</button>
                                </div>
                                <HelpCircleIcon className="h-24 w-24 text-red-600 mx-auto mb-4 animate-pulse" />
                                <h3 className="2xl font-bold text-red-600 mb-2">Emergency!</h3>
                                <p className="text-gray-700 mb-6">
                                    Tap to call the local tourist police or emergency services. Your location has been shared.
                                </p>
                                <button className="w-full bg-red-600 text-white font-bold py-4 px-6 rounded-xl shadow-lg hover:bg-red-700 transition-colors transform hover:scale-105">
...
//     const handleRegistration = (formData) => {
//         setIsSignUpModalOpen(false);
//         setUserProfile(formData); // Show profile after sign-up
//     };
